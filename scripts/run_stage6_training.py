...
"""
Stage 6 Training: 100h purerace, 300h contact NNs.

Same architecture and training pipeline as Stage 5, but with different hidden sizes:
  - PureRace: 100 hidden (was 200 in Stage 5), 196 Tesauro inputs
  - Racing/Attacking/Priming/Anchoring: 300 hidden (was 400 in Stage 5), 244 extended inputs

Uses same per-NN game plan weights as Stage 5 except Racing uses gpw=3.0
(gpw=5.0 diverges at 200h, safe at 400h; 300h uses conservative gpw=3.0).
...
Usage:
    python scripts/run_stage6_training.py                  # Full pipeline: TD + SL + score
    python scripts/run_stage6_training.py --sl-only        # Skip TD, use existing TD weights
    python scripts/run_stage6_training.py --score-only     # Just score existing weights
    python scripts/run_stage6_training.py --nn racing anchoring  # Train only specific NNs
"""
...
CONFIGS = {
    ...
    'racing': {
        'gpw': 3.0,  # gpw=5.0 diverges at 200h, safe at 400h; 300h uses conservative gpw=3.0
    ...
    'priming': {
        'gpw': 2.0,  # gpw=5.0 diverged at 300h; reduced to 2.0
        'phases': [(100, 20.0), (200, 10.0), (200, 3.1), (500, 1.0)],
    },
    'anchoring': {
        'gpw': 1.5,
...
def run_td_training():
    ...
    print(f'\n{"="*60}')
    print(f'  TD SELF-PLAY TRAINING (Stage 6)')
...
def score_stage6():
    """Score the Stage 6 trained models."""
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Stage 6 Training (100h purerace, 300h contact NNs)')
    ...
    scores = score_stage6()
    if scores:
        results_dir = os.path.join(project_dir, 'experiments', 'stage6')
